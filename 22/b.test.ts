...
function solution(input: string) {
    ...
    for (const i of bricks.keys()) {
        const queue = [...brickSupportedBy[i]].filter((j) => brickSupports[j].size === 1);
        ...
        while (queue.length) {
            ...
            if (!j) throw new Error("Unexpected");

            for (const k of brickSupportedBy[j]) {
                if (falling.has(k)) continue;

                if ([...brickSupports[k]].every((l) => falling.has(l))) {
                    queue.push(k);
                    falling.add(k);
                }
            }
